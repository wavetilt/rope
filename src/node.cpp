...
namespace rope
{
	...
	std::invalid_argument ERROR_OOB_NODE = std::invalid_argument("string index out of bounds");
	...
	rope_node::rope_node(std::string str)
		: weight_(str.length())
		, left_(nullptr)
		, right_(nullptr)
		, fragment_(std::move(str))
	{}
	 ...
	 , fragment_(aNode.fragment_)
	{
		...
		if (tmpLeft == nullptr) {
			...
			this->left_ = make_unique<rope_node>(*tmpLeft);
		}
	...
	string rope_node::treeToString() const {
		if (this->isLeaf()) {
			return this->fragment_;
		}
	...
	size_t rope_node::getDepth() const {
